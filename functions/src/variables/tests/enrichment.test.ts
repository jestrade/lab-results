...
/**
 * The categories as the collection has them, which is what enrichment
 * validates against now — the list is no longer compiled in, so the test has
 * to supply one the way `enrichVariables` supplies the real one.
 */
const CATEGORIES = new Set(['complete_blood_count', 'vitamins', 'iron_metabolism', 'other']);
...
describe('parseEnrichment', () => {
  it('keeps a well-formed entry with both translations', () => {
    const [entry] = parseEnrichment({ entries: [row()] }, REQUESTED, CATEGORIES);
  ...
  it('throws when the response has no entries array at all', () => {
    expect(() => parseEnrichment({}, REQUESTED, CATEGORIES)).toThrow();
    expect(() => parseEnrichment({ entries: 'none' }, REQUESTED, CATEGORIES)).toThrow();
  });
  ...
  it('drops a row missing either translation, leaving the placeholder in place', () => {
    ...
    expect(parseEnrichment({ entries: [row({ nameEs: '' })] }, REQUESTED, CATEGORIES)).toHaveLength(0);
    expect(parseEnrichment({ entries: [row({ nameEn: null })] }, REQUESTED, CATEGORIES)).toHaveLength(0);
  });

  it('falls back to "other" for a category the app does not have', () => {
    const [entry] = parseEnrichment({ entries: [row({ category: 'haematology' })] }, REQUESTED, CATEGORIES);
    expect(entry?.category).toBe('other');
  });

  it('accepts a recognised category', () => {
    const [entry] = parseEnrichment({ entries: [row({ category: 'vitamins' })] }, REQUESTED, CATEGORIES);
