...
describe('toCategory', () => {
  it('maps Spanish panel names to the app categories', () => {
    expect(toCategory('Biometría hemática')).toBe('complete_blood_count');
    expect(toCategory('Perfil de lípidos')).toBe('lipid_profile');
    expect(toCategory('Función renal')).toBe('kidney_function');
    expect(toCategory('Examen general de orina')).toBe('urinalysis');
  });

  it('maps English panel names too', () => {
    expect(toCategory('Complete Blood Count')).toBe('complete_blood_count');
    expect(toCategory('Liver function tests')).toBe('liver_function');
    expect(toCategory('Thyroid panel')).toBe('thyroid');
  });

  it('accepts a category id written directly in the sheet', () => {
    expect(toCategory('glucose_metabolism')).toBe('glucose_metabolism');
  });

  it('reads a group value labelled in both languages', () => {
    expect(toCategory('Hemograma / Complete Blood Count (CBC)')).toBe('complete_blood_count');
    expect(toCategory('Metabolismo y Glucosa / Metabolism and Glucose')).toBe(
      'glucose_metabolism',
    );
    expect(toCategory('Marcadores Inflamatorios / Inflammatory Markers')).toBe('inflammation');
  });
  ...
  it('files "Vitamins and Minerals" under vitamins, not electrolytes', () => {
    ...
    expect(toCategory('Vitaminas y Minerales / Vitamins and Minerals')).toBe('vitamins');
    expect(toCategory('Electrolitos / Electrolytes')).toBe('electrolytes');
  });
  ...
  it('falls back to "other" rather than picking the nearest group', () => {
    ...
    expect(toCategory('Marcadores tumorales')).toBe('other');
...
describe('readCatalog', () => {
  ...
  it('reads every variable with both names', () => {
    const { entries } = readCatalog(SHEET);
  ...
  it('carries both descriptions through', () => {
    const [hemoglobin] = readCatalog(SHEET).entries;
  ...
  it('omits a description nobody wrote rather than storing an empty one', () => {
    const hdl = readCatalog(SHEET).entries.find((entry) => entry.id === 'hdl-cholesterol');
  ...
  it('makes the abbreviation inside a curated name matchable on its own', () => {
    ...
    const [entry] = readCatalog(sheet).entries;
  ...
  it('keeps the parenthetical out of the id but the id readable', () => {
    ...
    expect(readCatalog(sheet).entries[0]?.id).toBe('urea');
  });
  ...
  it('carries a section-header row forward as the group', () => {
    ...
    const { entries } = readCatalog(sectioned);
  ...
  it('keeps both rows when the sheet lists a test twice', () => {
    ...
    const { entries } = readCatalog(duplicated);
  ...
  it('skips and counts rows with no name', () => {
    ...
    const { entries, skipped } = readCatalog(withJunk);
  ...
  it('imports a Spanish-only row under its Spanish name', () => {
    ...
    const [entry] = readCatalog(spanishOnly).entries;
  ...
  it('refuses a sheet with no name column at all', () => {
    expect(() => readCatalog('Grupo,Unidad\nBiometría,g/dL')).toThrow(/name column/i);
  });

  it('refuses an empty sheet', () => {
    expect(() => readCatalog('')).toThrow(/empty/i);
  });
});
