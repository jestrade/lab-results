...
describe('Variables', () => {
  ...
  describe('filters in the URL', () => {
    ...
    it('renders the page rather than an error when the URL is nonsense', async () => {
      renderPage('/variables?sort=zzz&period=99y');
      emit([potassium, makeSeries()]);
    ...
    it('shows an empty grid, not a broken one, for a category that does not exist', async () => {
      // No longer widened to "all": the ids are Firestore documents now, so
      // an unrecognised one is a filter that matches nothing rather than a
      // value the page can rule out. The reader gets the empty state and the
      // control to clear it.
      renderPage('/variables?category=made_up');
      emit([potassium, makeSeries()]);

      expect(await screen.findByText(/nothing matches/i)).toBeInTheDocument();
      expect(screen.getByText(/showing 0 of 2/i)).toBeInTheDocument();
      expect(screen.queryByRole('link', { name: /hemoglobin/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('link', { name: /potassium/i })).not.toBeInTheDocument();
    });
  });
