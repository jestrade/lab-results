...
const updateIdentityDocument = vi.hoisted(() => vi.fn());
const clearIdentityDocument = vi.hoisted(() => vi.fn());
...
describe('Profile', () => {
  beforeEach(() => {
    ...
    updateIdentityDocument.mockReset().mockResolvedValue(undefined);
    clearIdentityDocument.mockReset().mockResolvedValue(undefined);
  ...
  it('offers the four document types and no default selection', async () => {
    renderPage();
    emit(makeProfile());

    const select = await screen.findByLabelText(/document type/i);
    // Blank until the reader chooses. Defaulting to "Cédula" would be the app
    // stating something about them that they never did.
  ...
  it('saves the identity document the user entered', async () => {
    const user = userEvent.setup();
    renderPage();
    emit(makeProfile());

    await user.selectOptions(await screen.findByLabelText(/document type/i), 'pasaporte');
    await user.type(screen.getByLabelText(/document number/i), 'AQ123456');
    await user.type(screen.getByLabelText(/place of issue/i), 'Bogotá D.C.');
    await user.click(screen.getByRole('button', { name: /save document/i }));

    expect(updateIdentityDocument).toHaveBeenCalledWith('test-uid', {
      type: 'pasaporte',
      number: 'AQ123456',
      placeOfIssue: 'Bogotá D.C.',
    });
  });

  it('keeps the document number exactly as printed rather than reformatting it', async () => {
    const user = userEvent.setup();
    renderPage();
    emit(makeProfile());

    await user.selectOptions(await screen.findByLabelText(/document type/i), 'cedula');
    await user.type(screen.getByLabelText(/document number/i), '1.020.304.050');
    await user.click(screen.getByRole('button', { name: /save document/i }));

    // Identity numbers are printed differently across the four kinds, and a
    // helpful strip of the dots would store a number the card does not show.
    expect(updateIdentityDocument).toHaveBeenCalledWith(
      'test-uid',
      expect.objectContaining({ number: '1.020.304.050' }),
    );
  });

  it('refuses a document number with no type instead of storing loose digits', async () => {
    const user = userEvent.setup();
    renderPage();
    emit(makeProfile());

    await user.type(await screen.findByLabelText(/document number/i), '1020304050');
    await user.click(screen.getByRole('button', { name: /save document/i }));

    expect(updateIdentityDocument).not.toHaveBeenCalled();
    expect(await screen.findByText(/choose the document type/i)).toBeInTheDocument();
  });

  it('accepts a type on its own, because a half-remembered document is still true', async () => {
    const user = userEvent.setup();
    renderPage();
    emit(makeProfile());

    await user.selectOptions(await screen.findByLabelText(/document type/i), 'registro_civil');
    await user.click(screen.getByRole('button', { name: /save document/i }));
  ...
  it('writes null for a cleared place of issue rather than leaving the old one', async () => {
    const user = userEvent.setup();
    renderPage();
    emit(
      makeProfile({
    ...
    await user.clear(await screen.findByLabelText(/place of issue/i));
    await user.click(screen.getByRole('button', { name: /save document/i }));
  ...
  it('shows the stored document in the form rather than an empty one', async () => {
    renderPage();
    emit(
      makeProfile({
        identityDocument: {
    ...
    expect(await screen.findByLabelText(/document type/i)).toHaveValue('cedula_extranjeria');
    expect(screen.getByLabelText(/document number/i)).toHaveValue('E-4471129');
    expect(screen.getByLabelText(/place of issue/i)).toHaveValue('Cali');
  });

  it('confirms before removing the identity document', async () => {
    const user = userEvent.setup();
    renderPage();
    emit(
      makeProfile({
    ...
    await user.click(await screen.findByRole('button', { name: /remove my document/i }));
    expect(clearIdentityDocument).not.toHaveBeenCalled();

    await user.click(screen.getByRole('button', { name: 'Remove it' }));
    expect(clearIdentityDocument).toHaveBeenCalledWith('test-uid');
  });

  it('offers no document removal button when nothing is stored', async () => {
    renderPage();
    emit(makeProfile());

    await screen.findByRole('heading', { name: 'Identity document' });
    expect(screen.queryByRole('button', { name: /remove my document/i })).not.toBeInTheDocument();
  });
  ...
  it('marks the health context optional and denies it is a medical assessment', async () => {
    ...
    // Scoped to this section: the identity document is optional too, so the
    // tag now appears twice and an unscoped query would not say which one.
    const section = (await screen.findByRole('heading', { name: 'About you' })).closest(
      'section',
    ) as HTMLElement;
