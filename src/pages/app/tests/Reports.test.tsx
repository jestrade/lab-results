...
const updateReportDate = vi.hoisted(() => vi.fn());
...
describe('Reports', () => {
  beforeEach(() => {
    ...
    updateReportDate.mockReset();
    updateReportDate.mockResolvedValue(undefined);
  });
  ...
  describe('correcting the report date (KAN-13)', () => {
    async function openEditor() {
      const user = userEvent.setup();
      renderPage();
      emit([makeReport()]);
      await user.click(
        await screen.findByRole('button', { name: /change the report date for/i }),
      );
      return user;
    }

    it('opens on the date the row is showing', async () => {
      await openEditor();
      expect(screen.getByRole('dialog')).toHaveTextContent(/change the report date/i);
      expect(screen.getByLabelText(/date these tests were taken/i)).toHaveValue('2026-07-12');
    });

    it('saves the corrected day as UTC midnight', async () => {
      const user = await openEditor();

      fireEvent.change(screen.getByLabelText(/date these tests were taken/i), {
        target: { value: '2026-06-01' },
      });
      await user.click(screen.getByRole('button', { name: /save the date/i }));

      await waitFor(() => expect(updateReportDate).toHaveBeenCalled());
      const [reportId, date] = updateReportDate.mock.calls[0]! as [string, Date];
      expect(reportId).toBe('r1');
      // A day on paper, not an instant in the reader's zone.
      expect(date.toISOString()).toBe('2026-06-01T00:00:00.000Z');
    });

    it('refuses a date that has not happened yet, without writing anything', async () => {
      const user = await openEditor();
      ...
      expect(updateReportDate).not.toHaveBeenCalled();
    });

    it('says the results keep their old day until the report is processed again', async () => {
      await openEditor();
      // Correcting the date does not move the values that were already
      // extracted, and a dialog that stayed quiet about that would be
      // promising a change it does not make.
      expect(screen.getByRole('dialog')).toHaveTextContent(/processed again/i);
    });

    it('opens empty for a report that never had a date of its own', async () => {
      const user = userEvent.setup();
      renderPage();
      emit([makeReport({ reportDate: null })]);
      await user.click(
        await screen.findByRole('button', { name: /change the report date for/i }),
      );

      // Pre-filling with the upload date is how a wrong date gets confirmed
      // by a reader who is only skimming.
      expect(screen.getByLabelText(/date these tests were taken/i)).toHaveValue('');
    });

    it('reports a failed save rather than closing as if it worked', async () => {
      updateReportDate.mockRejectedValue(new Error('offline'));
      const user = await openEditor();
